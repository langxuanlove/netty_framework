...
public enum EventType {
	...
	/**
	 * Events of this event type should never occur. Not written to a binary
	 * log.
	 */
	UNKNOWN, /**
	...
	EXECUTE_LOAD_QUERY, /**
						 ...
						 * a table definition to a number, where the table
						 * definition consists of database and table names and
						 * column definitions. The purpose of this event is to
						 * enable replication when a table has different
						 ...
						 * TABLE_MAP events: one per table used by events in the
						 * sequence. Used in case of RBR.
						 */
	TABLE_MAP, /**
				 * Describes inserted rows (within a single table). Used in case
				 * of RBR (5.1.0 - 5.1.15).
				 */
	PRE_GA_WRITE_ROWS, /**
						 * Describes updated rows (within a single table). Used
						 * in case of RBR (5.1.0 - 5.1.15).
						 */
	PRE_GA_UPDATE_ROWS, /**
						 * Describes deleted rows (within a single table). Used
						 * in case of RBR (5.1.0 - 5.1.15).
						 */
	PRE_GA_DELETE_ROWS, /**
						 * Describes inserted rows (within a single table). Used
						 * in case of RBR (5.1.16 - mysql-trunk).
						 */
	WRITE_ROWS, /**
				 * Describes updated rows (within a single table). Used in case
				 * of RBR (5.1.16 - mysql-trunk).
				 */
	UPDATE_ROWS, /**
					 * Describes deleted rows (within a single table). Used in
	...
	EXT_DELETE_ROWS, /**
						 ...
						 */
	GTID, ANONYMOUS_GTID, PREVIOUS_GTIDS;
