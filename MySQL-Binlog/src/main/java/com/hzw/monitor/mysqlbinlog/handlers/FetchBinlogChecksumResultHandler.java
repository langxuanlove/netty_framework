	...
	protected void channelRead0(ChannelHandlerContext context, ByteBuf msg) throws Exception {
		try {
			...
			case WAITING_FOR_END:
				...
				if (0x00FE != firstByte) {
					...
					if (ChecksumType.NONE != checkSumType) {
						...
						new RequestBinaryLogCommand(p.getClientID(), myAttribute.getBinlogFileName(),
								myAttribute.getBinlogPosition()).write(context);
						// 这里要做一个变换
			...
			case SECOND_STAGE:
				...
				// 保留checkSumType
				MyAttributes myAttribute = context.channel().attr(MyConstants.MY_CONTEXT_ATTRIBUTES).get();
				myAttribute.setChecksumType(checkSumType);
				// 立即发送请求二进制日志命令
				MyProperties p = MyProperties.getInstance();
				new RequestBinaryLogCommand(p.getClientID(), myAttribute.getBinlogFileName(),
						myAttribute.getBinlogPosition()).write(context);
