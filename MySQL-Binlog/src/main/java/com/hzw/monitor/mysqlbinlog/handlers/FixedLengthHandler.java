...
public class FixedLengthHandler extends SimpleChannelInboundHandler<ByteBuf> {
	...
	protected void channelRead0(ChannelHandlerContext context, ByteBuf msg) throws Exception {
		try {
			{// 在上下文中创建一个变量方便值的传递
				if (false == initialized) {// 就不用每次都花很大精力去检查了
					InetSocketAddress remoteAddress = (InetSocketAddress) context.channel().remoteAddress();
					String ip = remoteAddress.getAddress().getHostAddress();
					int port = remoteAddress.getPort();
					LoggerUtils.debug(logger, "" + ip + " :" + port);
					context.channel().attr(MyConstants.MY_CONTEXT_ATTRIBUTES).set(new MyAttributes(ip, port,
							MyProperties.getInstance().getUsername(), MyProperties.getInstance().getPassword()));
					initialized = true;
				}
			}
			...
			while (index < length) {
				if (0 == headerReaded) {
					...
					// LoggerUtils.debug(logger, "full length " +
					// contentLength);
